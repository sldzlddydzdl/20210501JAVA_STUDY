...
public class 국민은행 extends 계좌 implements ATM {
	
	public static Scanner sc = new Scanner(System.in);
	
	public 국민은행(String 계좌, String 비밀번호, String 계좌주 , int 은행코드) {
	...
	public void 계좌목록() {
		for(int i = 0; i < Bankaccount.계좌리스트.size(); i++) {
				if( Bankaccount.계좌리스트.get(i).은행코드 == 1 ) {
					System.out.println("[국민은행] :  " + Bankaccount.계좌리스트.get(i).계좌 + " 의 잔액은 : " 
	...
	public int 인증( int 은행코드 ) {
		if(은행코드 == 1) {
			System.out.println(" 국민은행 인증 성공 ");
			return 1;
		}
		else {
			System.out.println("국민은행이 아닙니다.");
			return 0;
		}
		...
		for(int i = 0 ; i < Bankaccount.계좌리스트.size(); i++) {
			if(Bankaccount.계좌리스트.get(i).계좌.equals(계좌) && 
					Bankaccount.계좌리스트.get(i).비밀번호.equals(비밀번호) && 
						Bankaccount.계좌리스트.get(i).은행코드 == 1) {
					System.out.print("입금액 : ");
					int 입금액 = sc.nextInt();
					Bankaccount.계좌리스트.get(i).입금액 += 입금액;
					System.out.println(Bankaccount.계좌리스트.get(i).계좌주 + " 님 국민은행 입금 성공");
					return;
			}
		}	
	...
	public void 계좌생성() {
		...
		국민은행 temp = new 국민은행(계좌, 비밀번호 ,계좌주 , 1);
